...
def audio_numpy_concat(segment_data_list, sr, speed=1.):
    ...
    audio_segments = np.array(audio_segments).flatten().astype(np.float32)
    # 将范围[-1, 1]的float32数值转换到int16的范围
    int16_info = np.iinfo(np.int16)
    int16_array = (audio_segments / 1.414 * int16_info.max).astype(np.int16)
    
    return sr, int16_array
...
class MeloTTS:
    def __init__(self, model_path, language="ZH", speed=0.8):
        lower_lang = language.lower()
        encoder_path = os.path.join(model_path, f"encoder-{lower_lang}.onnx")
        decoder_path = os.path.join(model_path, f"decoder-{lower_lang}.onnx")
        self.encoder = ort.InferenceSession(encoder_path, providers=["CPUExecutionProvider"], sess_options=ort.SessionOptions())
        self.decoder = ort.InferenceSession(decoder_path, providers=["CPUExecutionProvider"], sess_options=ort.SessionOptions())

        self.speed = speed
        self._symbol_to_id = {s: i for i, s in enumerate(LANG_TO_SYMBOL_MAP[language])}
        self.dec_len = 65536 // 512
        # Load static input
        self.g = np.fromfile(os.path.join(model_path, f"g-{lower_lang}.bin"), dtype=np.float32).reshape(1, 256, 1)
    ...
    def tts(self, sentence):
        sens = split_sentences_into_pieces(sentence, self.language, quiet=True)
        # Final wav
        audio_list = []
        ...
        for n, se in enumerate(sens):
            ...
            sub_audio = merge_sub_audio(sub_audio_list, 0, audio_len)
            audio_list.append(sub_audio)
        return audio_numpy_concat(audio_list, sr=44100, speed=self.speed)
