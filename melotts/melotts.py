...
class MeloTTS:
    def __init__(self, model_path, language="ZH", speed=0.8):
        ...
        encoder_path = os.path.join(model_path, f"encoder-{language}.onnx")
        decoder_path = os.path.join(model_path, f"decoder-{language}.onnx")
        ...
        # Load static input
        self.g = np.fromfile(os.path.join(model_path, f"g-{language}.bin"), dtype=np.float32).reshape(1, 256, 1)
    ...
    def tts(self, sentence):
        ...
        for n, se in enumerate(sens):
            ...
            sub_audio = merge_sub_audio(sub_audio_list, 0, audio_len)
            audio_list.append(sub_audio)
        audio = audio_numpy_concat(audio_list, sr=44100, speed=self.speed)
        return audio
